	...
	<view>
		...
		<view class="history-all content-list" v-show="isShowList">
			...
			<view class="search-list">
				<view class="search-list_detial" v-for="(item, index) in historyList" :key="index" @click.stop="searchContents(item)">{{item}}</view>
			</view>
		</view>
		<!-- 历史搜索 end -->
		...
		<view class="content-list hot-topic" v-show="isShowList">
			...
			<view class="search-list">
				<view class="search-list_detial" :class="item.state == 1 ? 'active': ''" v-for="item in hotList" :key="item.id" @click.stop="hotTopic(item.id)">{{item.talk_theme}}</view>
			</view>
		</view>
		<!-- 热门话题 end -->
