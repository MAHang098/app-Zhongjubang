<template>
	<view class="content" v-bind:style="{height: clientHeight+ 'px'}">
		...
		<scroll-view class="scroll-content" v-bind:style="{height: (system.windowHeight)+ 'px'}" :scroll-y="true" :scroll-with-animation="true"  @scroll="scroll" @scrolltolower="scrolltolower" @scrolltoupper="refresh" upper-threshold="-50" :scroll-into-view="scrollToView"  @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd"   >
		
		<view v-for="(video,index) in videolist" :key="index" v-bind:id="'view-'+index" class="video-view" v-bind:style="{height: system.windowHeight + 'px'}"  >
				
			<video v-if="index == currentPage "  v-bind:id="'video-'+index" :src="video.video_url" v-bind:style="{height: (system.windowHeight-400*system.windowWidth/750)+ 'px'}" loop="true" :controls="false" :show-progress="false" :show-fullscreen-btn="false" :show-play-btn="false" :show-center-play-btn="false" :enable-progress-gesture="false" @tap="play(currentPage)" >
